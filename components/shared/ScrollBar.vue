...
import useVixtaEventListener from "../../composables/useVixtaEventListener";
...
export default {
  setup(props) {
    ...
    function setScrollbar() {
      ...
      killLastListener = useVixtaEventListener(window, "scroll", () => {
      ...
      useVixtaEventListener(window, "resize", () => {
        thumbHeight.value =
          (window.innerHeight / htmlEl.scrollHeight) * window.innerHeight;
        thumbHeight.value =
          thumbHeight.value === window.innerHeight ? 0 : thumbHeight.value;
...
.scrollbar__thumb {
  width: 6px;
  background: var(--vixta-dark-brown);
  border-radius: 8px;
